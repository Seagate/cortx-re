...
cleanup() {
  ...
  output=$(s3iamcli resetaccountaccesskey -n cortxaccount --ldapuser sgiamadmin --ldappasswd "$ldappasswd" --no-ssl) 2> /dev/null || echo ""
  if [[ $output == *"Name or service not known"*  || $output == *"Invalid argument"* ]];then
    ...
    s3iamcli deleteuser -n cortxuser --access_key "$access_key" --secret_key "$secret_key" --no-ssl 2> /dev/null || echo ""
    s3iamcli deleteaccount -n cortxaccount --access_key "$access_key" --secret_key "$secret_key" --no-ssl 2> /dev/null || echo ""
  fi
...
echo "Create Account"
output=$(s3iamcli createaccount -n cortxaccount  -e cortxaccount@sanitybucket.com --ldapuser sgiamadmin --ldappasswd "$ldappasswd" --no-ssl)
access_key=$(echo -e "$output" | tr ',' '\n' | grep "AccessKeyId" | awk '{print $3}')
secret_key=$(echo -e "$output" | tr ',' '\n' | grep "SecretKey" | awk '{print $3}')
s3iamcli CreateUser -n cortxuser --access_key "$access_key" --secret_key "$secret_key" --no-ssl 2> /dev/null
if [ $? -ne 0 ];then
  restore_config
...
echo "Delete User - 'cortxuser': "
s3iamcli deleteuser -n cortxuser --access_key "$access_key" --secret_key "$secret_key" --no-ssl 2> /dev/null
if [ $? -ne 0 ];then
  anys3iamclitestsfailed=true
  echo "s3iamcli deleteuser failed"
fi

echo "Delete Account - 'cortxaccount': "
s3iamcli deleteaccount -n cortxaccount --access_key "$access_key" --secret_key "$secret_key" --no-ssl
if [ $? -ne 0 ];then
  anys3iamclitestsfailed=true
