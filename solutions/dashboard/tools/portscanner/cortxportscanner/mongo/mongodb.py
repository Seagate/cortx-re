...
class MongoDB:
    ...
    def update_document(self, data):
        try:
            ...
            resp = self.collection.update_one({"_id": self.document_id}, update={"$set": data})
            print("Response - ", resp, " -> ", resp.acknowledged)
        except Exception as err:
            print("Update Document Exception: ", err)

    def manage_document(self, actual_ports: list = [], allowed_ports: list = [], non_compliance_ports: list = [], non_compliance_services:list = [], is_healthy: bool = True, is_healthy_int:int = 1):
        try:
            data = {
