...
function docker_check() {
        ...
        if [ $? -eq 0 ]; then
                ...
                yum install -y yum-utils && yum-config-manager --add-repo https://download.docker.com/linux/centos/docker-ce.repo -y && yum install -y docker-ce docker-ce-cli containerd.io docker-compose-plugin && sleep 30 && systemctl start docker && systemctl enable docker
        fi
}
docker_check
...
#Install git and jq
add_primary_separator "Installing git and jq"
yum install git epel-release -y && yum install jq -y
