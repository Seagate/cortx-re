pipeline {
  ...
  //       Scheduled to run on daily ~ 1-2 AM IST
    ...
    parameters {  
        ...
        string(name: 'CODACY_URL', defaultValue: 'https://app.codacy.com/gh/Seagate/cortx-s3server/dashboard', description: 'Codacy Code Coverage URL', trim: true)
        string(name: 'COMPONENT', defaultValue: 'CORTX-S3Server', description: 'component name', trim: true)
	}
			    ...
			    try {
                    sh label: 'download_log_files', returnStdout: true, script: """ 
                       # pip3 install json2html
						python3 CodacyCodeCoverage.py cortx-s3server > s3server.html
                    """
                } catch (err) {
                    echo err.getMessage()
                }
