...
pipeline { 
    ...
    parameters {
        ...
        choice(name: 'MAXIMUM_ALLOWED_TIME_TO_HOLD_NODE', choices: [ 2, 6, 12, 24 ], description: 'Only enable & cleanup the node that was disabled X hours before. FORCE option will ovverride this behaviour')
...
def enableOfflineVM(nodeLabel) {
    ...
    for (node in Jenkins.instance.nodes) {
        ...
        if ( node_label.contains("cleanup_req") && ( node_label.contains("vm_deployment_3n"))
