...
pipeline {
		...
		stage('Checkout Script') {
            steps {             
                script {
                    checkout([$class: 'GitSCM', branches: [[name: 'releaseinfo-commit-tag']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'cortx-admin-github', url: 'https://github.com/Seagate/cortx-re']]])                
                }
            }
        }
		stage ("Tagging") {
			steps {
				script { build_stage=env.STAGE_NAME }
					script { 
						...
						sh label: 'Git-Tagging', script: '''sh -x scripts/release_support/git-tag.sh $RELEASE_INFO_URL $GIT_TAG $TAG_MESSAGE'''  
					}
				}			
			}
		}
