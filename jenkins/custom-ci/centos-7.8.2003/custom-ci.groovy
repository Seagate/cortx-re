...
pipeline {
    ...
    environment {
        ...
        third_party_dir = "${THIRD_PARTY_RPM_VERSION == 'cortx-2.0' ? "$release_dir/third-party-deps/centos/centos-7.8.2003-2.0.0-new" : THIRD_PARTY_RPM_VERSION == 'cortx-1.0' ?  "$release_dir/third-party-deps/centos/centos-7.8.2003-1.0.0-1" : "$release_dir/third-party-deps/centos/centos-7.8.2003-custom"}"
    }
    ...
    stages {
                ...
                sh label: 'Copy RPMS', script:'''
                    ...
                    do
                        ...
                        if ls $component/last_successful/*.rpm 1> /dev/null 2>&1; then
                            cp $component/last_successful/*.rpm $integration_dir/$release_tag/sw/cortx
                        else
        ...
        stage('RPM Validation') {
            steps {
                ...
                do
                    ...
                    echo "-------------------------------------"
                    pushd $integration_dir/$release_tag/sw/cortx/
        ...
        stage ('Link external rpm and python deps') {
            steps {
                script { build_stage = env.STAGE_NAME }
                sh label: 'Tag Release', script: '''
                    mkdir -p $integration_dir/$release_tag/sw/external && pushd $integration_dir/$release_tag/sw/external
                        ln -s $(readlink -f $third_party_dir) rpm
                        ln -s $(readlink -f $python_deps) python
                    popd
                '''
            }
        }
        ...
        stage ('Generate ISO Image') {
            steps {
                ...
                #Add cortx-prep.sh
                cortx_prvsnr_preq=$(ls "$integration_dir/$release_tag/sw/cortx" | grep "python36-cortx-prvsnr" | cut -d- -f5 | cut -d_ -f2 | cut -d. -f1 | sed s/"git"//)
