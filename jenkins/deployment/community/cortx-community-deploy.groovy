pipeline {
    ...
    parameters {
        ...
        string(name: 'EBS_VOLUME_COUNT', defaultValue: '9', description: 'EBS volumes to attach onto nodes. Currently maximum 26 disks are supported', trim: true)
            ...
            stage ('Install Tools') {
                steps {
                    ...
                    pushd solutions/community-deploy/cloud/AWS
                        ./tool_setup.sh
                        sed -i 's,os_version =.*,os_version = "'"$OS_VERSION"'",g' user.tfvars && sed -i 's,region =.*,region = "'"$REGION"'",g' user.tfvars && sed -i 's,security_group_cidr =.*,security_group_cidr = "'"$VM_IP/32"'",g' user.tfvars && sed -i 's,ebs_count =.*,ebs_count = "'"$EBS_VOLUME_COUNT"'",g' user.tfvars && sed -i 's,ebs_size =.*,ebs_size = "'"$EBS_VOLUME_SIZE"'",g' user.tfvars
    ...
    post {
        ...
        always {
            retry(count: 3) {
            script { build_stage = env.STAGE_NAME }
            sh label: 'Destroy EC2 instance', script: '''
            pushd solutions/community-deploy/cloud/AWS
