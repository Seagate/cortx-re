        ...
        stage('Generate locally build cortx images') {
            steps {
                ...
                pushd solutions/community-deploy/cloud/AWS
                    ...
                    for wp in $WORKER_IP;do ssh -i cortx.pem -o 'StrictHostKeyChecking=no' centos@${wp} "sudo docker pull '${CORTX_SERVER_IMAGE}' && sudo docker pull '${CORTX_DATA_IMAGE}' && sudo docker pull '${CORTX_CONTROL_IMAGE} && sleep 60'";done
                    popd
            '''
            }
        }
