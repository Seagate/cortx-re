        ...
        stage('Setup K8s cluster') {
            steps {
                ...
                pushd solutions/community-deploy/cloud/AWS
                    ...
                    sleep 240
                    for wp in $WORKER_IP;do ssh -i cortx.pem -o 'StrictHostKeyChecking=no' centos@${wp} "sudo -- sh -c 'sed -i 's,cortx-docker.colo.seagate.com,${HOST1}:8080,g' /etc/docker/daemon.json && systemctl restart docker && sleep 240 && docker pull ${HOST1}:8080/seagate/cortx-rgw:2.0.0-0 && docker pull ${HOST1}:8080/seagate/cortx-data:2.0.0-0 && docker pull ${HOST1}:8080/seagate/cortx-control:2.0.0-0'";done
                    popd
            '''
            }
        }
