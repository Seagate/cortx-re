	...
	stages {
		...
		stage ('Integrate Component RPM') {
			steps {
                ...
                sh label: 'Copy RPMS', script:'''
                    ...
                    mkdir -p $integration_dir/$release_name/dev
                    mkdir -p $integration_dir/$release_name/prod
                    ...
                    pushd $premerge_component_dir
                        for component in $(echo $premerge_comp | tr "|" "\n"); do 
                            ...
                            cp -R $component_real_build_path/*.rpm $integration_dir/$release_name/dev
                            cp -R $component_real_build_path/*.rpm $integration_dir/$release_name/prod
                        done    
                    popd
			...
			steps {
				...
				sh label: 'Validate RPMS for Motr Dependency', script:''' 
                    ...
                    do
                        ...
                        set +x
                        pushd $integration_dir/$release_name/$env
		...
		stage ('Sign rpm') {
			steps {
                ...
                sh label: 'Sign rpm', script: '''
                    cp RPM-GPG-KEY-Seagate $integration_dir/$release_name/dev
                    cp RPM-GPG-KEY-Seagate $integration_dir/$release_name/prod
                    pushd scripts/rpm-signing
                        chmod +x rpm-sign.sh
                        for rpm in `ls -1 $integration_dir/$release_name/dev/*.rpm`
                        do
                            ./rpm-sign.sh ${passphrase} $rpm
                        done

                        for rpm in `ls -1 $integration_dir/$release_name/prod/*.rpm`
                        do
                            ./rpm-sign.sh ${passphrase} $rpm
                        done
                    popd
        ...
        stage ('Host RRPM (Create Repo)') {
			steps {
                ...
                sh label: 'Repo Creation', script: '''
                    pushd $integration_dir/$release_name/dev
                    rpm -qi createrepo || yum install -y createrepo
                    createrepo .
                    popd
                    pushd $integration_dir/$release_name/prod
                    rpm -qi createrepo || yum install -y createrepo
                    createrepo .
                    popd
                '''		
                ...
                sh label: 'Release cortx_build', script: '''
					pushd $integration_dir
                        ...
                        mkdir $release_name && pushd $release_name
                            ln -s $integration_dir/$release_name/prod cortx_iso
			...
			steps {
                ...
                sh label: 'Build MANIFEST', script: """
                    ...
                    pushd scripts/release_support
                        ...
                        sh build_release_info.sh $integration_dir/$release_name/dev                    
                        sh build_readme.sh $integration_dir/$release_name
                        sh build-3rdParty-release-info.sh $integration_dir/$release_name/3rd_party
                    
                    popd

                    cp $integration_dir/$release_name/README.txt .
                    cp $integration_dir/$release_name/dev/RELEASE.INFO .
                """		
                 withCredentials([string(credentialsId: 'shailesh-github-token', variable: 'ACCESS_TOKEN')]) {
                    sh label: 'Generate Changelog', script: """
                        ...
                        cp CHANGESET.txt $integration_dir/$release_name/dev
                        cp CHANGESET.txt $integration_dir/$release_name/prod
                    """	
            }
		}
        ...
        stage ('Generate ISO Image') {
		    steps {
		        ...
		        pushd $iso_location
					genisoimage -input-charset iso8859-1 -f -J -joliet-long -r -allow-lowercase -allow-multidot -publisher Seagate -o $branch-$BUILD_NUMBER.iso $integration_dir/$release_name/prod
				popd
				
				genisoimage -input-charset iso8859-1 -f -J -joliet-long -r -allow-lowercase -allow-multidot -publisher Seagate -o cortx-$version-$BUILD_NUMBER-single.iso $integration_dir/$release_name
				
                mkdir $integration_dir/$release_name/iso
                ln -s $cortx_os_iso $integration_dir/$release_name/iso/$(basename $cortx_os_iso)

                cortx_prvsnr_preq=$(ls "$integration_dir/$release_name/cortx_iso" | grep "python36-cortx-prvsnr" | cut -d- -f5 | cut -d_ -f2 | cut -d. -f1 | sed s/"git"//)
                wget -O $integration_dir/$release_name/iso/cortx-prep-$version-$BUILD_NUMBER.sh https://raw.githubusercontent.com/Seagate/cortx-prvsnr/$cortx_prvsnr_preq/cli/src/cortx_prep.sh
            
				cp cortx-$version-$BUILD_NUMBER-single.iso $integration_dir/$release_name/iso
				
				ls $integration_dir/$release_name/3rd_party/THIRD_PARTY_RELEASE.INFO
				ls $integration_dir/$release_name/cortx_iso/RELEASE.INFO
				
				cp $integration_dir/$release_name/3rd_party/THIRD_PARTY_RELEASE.INFO $integration_dir/$release_name
				cp $integration_dir/$release_name/cortx_iso/RELEASE.INFO $integration_dir/$release_name
				sed -i '/BUILD/d' $integration_dir/$release_name/3rd_party/THIRD_PARTY_RELEASE.INFO
		        '''
		    }
		}
