		...
		stage ('Sign rpm') {
			steps {
				...
				'''
					...
					pushd scripts/rpm-signing
        	            chmod +x rpm-sign.sh
                	    cp RPM-GPG-KEY-Seagate $integration_dir/$release_tag/cortx_build_temp/dev
	                    cp RPM-GPG-KEY-Seagate $integration_dir/$release_tag/cortx_build_temp/prod

        	            for rpm in `ls -1 $integration_dir/$release_tag/cortx_build_temp/dev/*.rpm`
                	    do
	                    ./rpm-sign.sh ${passphrase} $rpm
        	            done
	
        	            for rpm in `ls -1 $integration_dir/$release_tag/cortx_build_temp/prod/*.rpm`
                	    do
	                    ./rpm-sign.sh ${passphrase} $rpm
        	            done
					popd
		...
		stage ('Repo Creation') {
			steps {
	                ...
	                sh label: 'Repo Creation', script: '''
        	            pushd $integration_dir/$release_tag/cortx_build_temp/dev
