...
yes -> Preserve host for troublshooting [ WARNING ! Automated Deployment May be queued/blocked if more number of vm used for debuging ]  
        ...
        string(name: 'NODE1', defaultValue: '', description: '''<pre>
FQDN of ssc-vm
...
- Additional Disk Size  : 25 GB  
        ...
        environment {
            ...
            NODE1_HOST = "${params.NODE1.isEmpty() ? NODE1_HOST : params.NODE1 }"
            NODE2_HOST = "${params.NODE2.isEmpty() ? NODE2_HOST : params.NODE2 }"
            NODE3_HOST = "${params.NODE3.isEmpty() ? NODE3_HOST : params.NODE3 }"
        ...
        post { 
            always {
                script {
                    ...
                    def mailRecipients = "nilesh.govande@seagate.com, basavaraj.kirunge@seagate.com, rajesh.nambiar@seagate.com, amit.kumar@seagate.com,CORTX.s3@seagate.com"
                    emailext body: '''${SCRIPT, template="mini_prov-email.template"}''',
                    mimeType: 'text/html',
                    recipientProviders: [requestor()], 
                    subject: "[Jenkins] Openldap 3N MiniProvisioning : ${currentBuild.currentResult}, ${JOB_BASE_NAME}#${BUILD_NUMBER}",
                    to: "${mailRecipients}"
