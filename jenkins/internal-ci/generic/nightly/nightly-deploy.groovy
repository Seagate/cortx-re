            ...
            steps {
                script {
                    catchError(stageResult: 'FAILURE') {
                        ...
                        env.Regression_overall_failed = = qaSanity.buildVariables.Regression_overall_failed
        ...
        always {
            script {
                ...
                if ( "${env.cortxCluster_status}" == "SUCCESS" && "${env.qaSanity_status}" == "SUCCESS" ) {
                    ...
                    MESSAGE = "K8s Build#${build_id} ${env.numberofnodes}Node Deployment Deployment=failed, SanityTest=${env.Sanity_status}, Regression=${env.Regression_overall_failed}"
                    ICON = "error.gif"
                    STATUS = "FAILURE"
                    ...
                    manager.buildFailure()
                    MESSAGE = "K8s Build#${build_id} ${env.numberofnodes}Node Deployment Deployment=Passed, SanityTest=${env.Sanity_status}, Regression=${env.Regression_overall_failed}"
                    ICON = "error.gif"
                    STATUS = "FAILURE"
