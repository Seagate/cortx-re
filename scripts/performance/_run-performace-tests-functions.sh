...
function execute_perfpro() {
    ...
    pushd $SCRIPT_LOCATION/performance/PerfPro
        ...
        ANSIBLE_LOG_PATH=$ANIBLE_LOG_FILE ansible-playbook perfpro.yml -i inventories/hosts --extra-vars "{ \"EXECUTION_TYPE\" : \"sanity\" ,\"REPOSITORY\":[{ \"category\": \"motr\", \"repo\": \"cortx-motr\", \"branch\": \"k8s\", \"commit\": \"a1234b\" }, { \"category\": \"rgw\", \"repo\": \"cortx-rgw\", \"branch\": \"dev\", \"commit\": \"c5678d\" }, { \"category\": \"hare\", \"repo\": \"cortx-hare\", \"branch\": \"main\", \"commit\": \"e9876f\" }],\"PR_ID\" : \"cortx-rgw/1234\" , \"USER\":\"JENKINS\",\"GID\" : \"00000\", \"NODES\":{\"1\": \"$PRIMARY_NODE\"} , \"CLIENTS\":{\"1\": \"$CLIENT_NODE\"} , \"main\":{\"db_server\": \"$DB_SERVER\", \"db_port\": \"$DB_PORT\", \"db_name\": \"$DB_NAME\", \"db_user\": \"$DB_USER\", \"db_passwd\": \"$DB_PASSWD\", \"$DB_DATABASE\": \"performance_database\", \"db_url\": \"mongodb://$DB_USER:$DB_PASSWD@$DB_SERVER:$DB_PORT/\"}, \"config\":{\"CLUSTER_PASS\": \"$PRIMARY_CRED\", \"END_POINTS\": \"$ENDPOINT_URL\", \"CUSTOM\" : \"VM\" }}" -v
    popd
}
