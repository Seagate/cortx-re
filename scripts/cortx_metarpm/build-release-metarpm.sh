...
`curl http://cortx-storage.colo.seagate.com/releases/eos/github/$release_folder/$OS_version/$build_num/prod/ | sed -n "/rpm/p" | cut -d'"' -f 2 > metainfo.txt`
...
builddep=$(cat dependencies.txt | tr '\n' ' ')
