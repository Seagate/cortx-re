...
declare -A REPO_LIST=(
		...
		COMMIT_HASH="$(cat release.info | grep -w '*.rpm\|"$component"\|uniq' | awk '!/debuginfo*/' | awk -F['_'] '{print $2}' | cut -d. -f1 |  sed 's/git//g' | grep -v ^[[:space:]]*\$";
		...
		COMMIT_HASH="$(cat release.info | grep -w '*.rpm\|"$component"\|uniq' | awk '!/debuginfo*/' | awk -F['_'] '{print $3}' | cut -d. -f1 |  sed 's/git//g' | grep -v ^[[:space:]]*\$";
                else
