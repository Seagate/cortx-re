...
source "$WORKSPACE"/solutions/kubernetes/functions.sh "$WORKSPACE"/solutions/kubernetes/cortx-deploy-functions.sh
...
SCRIPT_LOCATION="/root/cortx-k8s/k8_cortx_cloud"
...
function upgrade_cluster() {
    ...
    for i in "${!imageArray[@]}"; do update_image "${servicesArray[i]}" "${imageArray[i]}" "/root/cortx-k8s/k8_cortx_cloud"; done &&
